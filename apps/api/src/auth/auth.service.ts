...
class InMemoryRedis {
  ...
  async incr(key: string): Promise<number> {
    ...
    this.store.set(key, {
      value: val.toString(),
      expiresAt: item?.expiresAt || Date.now() + 900000,
    });
    return val;
  }
...
export class AuthService {
  ...
  async handleFailedLogin(
    ...
    this.logger.warn(
      `Failed ${portal} login attempt for user ${email} from IP ${ip}. Count: ${failures}`,
    );
  ...
  async validateUser(
    ...
    if (portal === 'admin') {
      ...
      const userRoleNames = user.roles.map((r: any) => r.name);
      const hasAdminPortalRole = userRoleNames.some((r: string) =>
        ADMIN_PORTAL_ROLES.includes(r),
      );
      ...
      if (hasAdminPortalRole) {
        return rejectGeneric(
          'admin users must log in through the admin portal',
        );
      }
    }
  ...
  async rotateRefreshToken(
    ...
    if (isExpired || isRevoked) {
      if (isRevoked) {
        this.logger.error(
          `Refresh token reuse detected for user ${dbToken.userId}! Revoking token family.`,
        );
  ...
  async requestPasswordReset(email: string, ip: string): Promise<void> {
    ...
    this.logger.log(
      `Password reset pending admin approval for ${email} (userId: ${user.id})`,
    );
  ...
  async approvePendingReset(
    ...
    if (!pending) {
      throw new BadRequestException(
        'No pending reset request found for this user',
      );
    }
    ...
    this.logger.log(
      `Admin ${actorId} approved password reset for user ${userId}. Link: ${resetLink}`,
    );
  ...
  async getPendingResets(): Promise<
    Array<{ userId: string; email: string; name: string; requestedAt: string }>
  > {
    ...
    for (const log of recentResets) {
      ...
      const pending = await this.redis.get(
        `password_reset_pending:${log.resourceId}`,
      );
      if (pending) {
  ...
  private async generateInviteToken(
    userId: string,
  ): Promise<{ inviteLink: string; tokenHash: string }> {
  ...
  async signUp(
    ...
    this.logger.log(
      `New sign-up request from ${dto.email} — awaiting admin approval.`,
    );
    return {
      message: 'Registration request submitted. Awaiting admin approval.',
    };
