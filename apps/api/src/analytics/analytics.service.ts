...
export class AnalyticsService {
  ...
  async getDepartmentPerformance(userVerticals: string[]) {
    ...
    const [targets, snapshots] = await Promise.all([
      this.prisma.target.findMany({ where }),
      this.prisma.targetSnapshot.findMany({
        where: {
          target: where,
        },
        include: {
    ...
    const historyMap: Record<
      string,
      Record<string, { total: number; currentValue: number }>
    > = {};
    ...
    const historyArray = Object.entries(historyMap).map(([date, depts]) => ({
      ...
      Sales:
        depts.Sales.total > 0
          ? depts.Sales.currentValue / depts.Sales.total
          : 0,
      Production:
        depts.Production.total > 0
          ? depts.Production.currentValue / depts.Production.total
          : 0,
      Hiring:
        depts.Hiring.total > 0
          ? depts.Hiring.currentValue / depts.Hiring.total
          : 0,
    }));
  ...
  async getTrends(userVerticals: string[]) {
    ...
    const historicalMix = Object.entries(historicalMixMap).map(
      ([date, counts]) => ({
        date,
        ...counts,
      }),
    ...
    for (const t of targets) {
      ...
      if (stats.actualProgress >= 1) {
        ...
        const durationDays = Math.ceil(
          (t.deadline.getTime() - t.startDate.getTime()) /
            (1000 * 60 * 60 * 24),
        );
      ...
      if (new Date(t.deadline) < now && stats.actualProgress < 1) {
        ...
        const delay = Math.ceil(
          (now.getTime() - t.deadline.getTime()) / (1000 * 60 * 60 * 24),
        );
          ...
          ? Math.round(
              completedCount /
                Math.max(
                  ...
                  Math.ceil(
                    (now.getTime() -
                      Math.min(...targets.map((t) => t.startDate.getTime()))) /
                      (1000 * 60 * 60 * 24 * 30),
                  ),
                ),
            )
  ...
  async getDepartmentBreakdown(userVerticals: string[]) {
    ...
    for (const t of targets) {
      ...
      deptMap[dept].total++;
      deptMap[dept].totalProgress += Math.max(
        0,
        Math.min(100, stats.actualProgress * 100),
      );
    ...
    return Object.entries(deptMap).map(([dept, data]) => ({
      ...
      ...data,
      avgProgress:
        data.total > 0 ? Math.round(data.totalProgress / data.total) : 0,
    }));
  }
    ...
    for (const t of targets) {
      ...
      if (!ownerMap[owner])
        ownerMap[owner] = {
          ...
          total: 0,
          totalProgress: 0,
          department: t.vertical,
        };
      ownerMap[owner].total++;
      ownerMap[owner].totalProgress += Math.max(
        0,
        Math.min(100, stats.actualProgress * 100),
      );
  ...
  async getDeadlines(userVerticals: string[]) {
    ...
    const upcoming = targets
      .filter(
        (t) =>
          new Date(t.deadline) >= now && new Date(t.deadline) <= next30Days,
      )
      .slice(0, 20)
      .map((t) => {
        const stats = calculateRagStatus(
        ...
        return {
          ...
          progress: Math.round(stats.actualProgress * 100),
        };
      });
  ...
  async getHeatmapData(userVerticals: string[]) {
    ...
    const dayMap: Record<
      string,
      { updates: number; green: number; amber: number; red: number }
    > = {};
    for (const snap of snapshots) {
      const dateStr = snap.capturedAt.toISOString().split('T')[0];
      if (!dayMap[dateStr])
        dayMap[dateStr] = { updates: 0, green: 0, amber: 0, red: 0 };
  ...
  async getInsights(userVerticals: string[]) {
    ...
    const deptMap: Record<
      string,
      { green: number; total: number; overdue: number }
    > = {};
    for (const t of targets) {
      const stats = calculateRagStatus(
        t.startDate,
    ...
    for (const [dept, data] of Object.entries(deptMap)) {
      const greenPct =
        data.total > 0 ? Math.round((data.green / data.total) * 100) : 0;
      ...
      if (behindPct > 50) {
        insights.push({
          ...
          description: `${dept} is ${behindPct}% behind schedule with only ${greenPct}% of targets on track.`,
          metric: `${behindPct}% behind`,
        });
      } else if (greenPct >= 80) {
        insights.push({
          type: 'success',
          title: `${dept} Exceeding Targets`,
          description: `${dept} has ${greenPct}% of targets on track — excellent performance!`,
          metric: `${greenPct}% on track`,
        });
      }
      ...
      if (data.overdue > 2) {
        insights.push({
          type: 'warning',
          title: `${data.overdue} Overdue Targets in ${dept}`,
          description: `${dept} has ${data.overdue} overdue targets that require immediate attention.`,
          metric: `${data.overdue} overdue`,
        });
      }
    }
    ...
    insights.push({
      ...
      description: `${overallGreenPct}% of all targets are currently on track across all departments.`,
      metric: `${overallGreenPct}%`,
    });
    ...
    const next7 = targets.filter((t) => {
      const dl = new Date(t.deadline);
      return (
        dl >= now && dl <= new Date(now.getTime() + 7 * 24 * 60 * 60 * 1000)
      );
    }).length;
    if (next7 > 0) {
      insights.push({
        ...
        description: `${next7} target${next7 > 1 ? 's' : ''} are due within the next 7 days. Ensure teams are prepared.`,
        metric: `${next7} due soon`,
      });
    }
